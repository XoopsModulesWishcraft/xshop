 ...
 * @access 		public
 */
class XoopsFormSelectShipping extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectShipping($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
		...
		foreach($shipping_handler->getObjects($criteria, true) as $shipping_id => $item) {
			$digest = $item_digest_handler->getItem($item->getVar('items_id'));
			$this->addOption($shipping_id, $digest->getVar('menu_title'));
		}
    }
