 ...
 * @access 		public
 */
class XShopFormSelectAddress extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XShopFormSelectAddress($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
        ...
        $criteria->setSort('`type`, `country_id`');
        
		foreach($addresses_handler->getObjects($criteria, true) as $addresses_id => $item)
			$this->addOption($addresses_id, $item->getVar('address_line_1').', '.$item->getVar('suburb').', '.$item->getVar('city'));
    }
