...
class xshopGallery extends XoopsObject 
{
    ...
    function __construct()
    {
        ...
        $this->initVar('thumbnail_path', XOBJ_DTYPE_TXTBOX, DS.$xoModule->getVar('dirname').DS.md5(microtime()).DS.'thumbnail'.DS, 255);
    ...
    function createFolders() {
    	$this->createPath(XOOPS_UPLOAD_PATH.$this->getVar('path'));
    	$this->createPath(XOOPS_UPLOAD_PATH.$this->getVar('thumbnail_path'));
    	return true;	
    }
    ...
    function getForm($querystring, $captions = true, $render = true, $index = '', $cursor = 'form', $frmobj = '', $const = '_SHOP_MI_GALLERY') {
    	...
    	if ($render == true||$captions==true) {
	    	...
	    	$frmobj[$cursor]['upload'] = new XoopsFormFile(_SHOP_FRM_GALLERY_UPLOAD_FILE, str_replace(']', '', str_replace('[', '_', $id.'[upload]')), $GLOBALS['xoopsModuleConfig']['max_file_size']);
...
class xshopGalleryHandler extends XoopsPersistableObjectHandler
{
    ...
    function insert($object, $force = true, $create_thumb=false) {
    	...
    	if ($create_thumb==true) {
    		...
    		WideImage::load(XOOPS_UPLOAD_PATH.$object->getVar('path').$object->getVar('filename'))->resize($GLOBALS['xoopsModuleConfig']['thumbnail_width'], $GLOBALS['xoopsModuleConfig']['thumbnail_height'])->saveToFile(XOOPS_UPLOAD_PATH.$object->getVar('thumbnail_path').$object->getVar('filename'));
    	}
