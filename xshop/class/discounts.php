...
class xshopDiscounts extends XoopsObject 
{
    ...
    function isOpen() {
    	if ($this->getVar('timed')) {
    		if ($this->getVar('start')<=time()&&$this->getVar('end')>=time()) {
    			$diff = time()-strtotime(date('Y-m-d'));
    			if ($this->getVar('opening')<=$diff&&$this->getVar('closed')>=$diff) {
    				return true;
    			} else {
    				return false;
    			}
    ...
    function getDiscount($quanity, $product) {
    	$days_handler = xoops_getmodulehandler('days', 'xshop');
    	$days = $days_handler->get($this->getVar('days_id'));
    	if (is_object($days)) {
	    	if ($days->getVar(strtolower(date('l')))=='_SHOP_MI_DAYS_DISCOUNT') {
		    	if ($this->isOpen()) {
	    			switch($this->getVar('type'))
	    			{
	    				case '_SHOP_MI_DISCOUNT_QUITSTOCK':
	    					if ($product->getVar('stock')=='_SHOP_MI_PRODUCTS_QUITSTOCK') {
	    						return array('percentage'=>$this->getVar('percentage'), 'discount_id'=>$this->getVar('discount_id'), 'shipping_id'=>$this->getVar('shipping_id'), 'amount' => ($product->getTotal($quanity) * ($this->getVar('percentage')/100)));
	    					}
	    					break;
	    				case '_SHOP_MI_DISCOUNT_GENERAL':
	    					return array('percentage'=>$this->getVar('percentage'), 'discount_id'=>$this->getVar('discount_id'), 'shipping_id'=>$this->getVar('shipping_id'), 'amount' => ($product->getTotal($quanity) * ($this->getVar('percentage')/100)));
	    					break;
	    				case '_SHOP_MI_DISCOUNT_WHOLESALE':
	    					if ($product->getVar('quanity_for_wholesale')>=$quanity&&$product->getVar('quanity_for_wholesale')>0) {
	    						return array('percentage'=>$this->getVar('percentage'), 'discount_id'=>$this->getVar('discount_id'), 'shipping_id'=>$this->getVar('shipping_id'), 'amount' => ($product->getTotal($quanity) * ($this->getVar('percentage')/100)));
	    					}
	    					break;
	    				case '_SHOP_MI_DISCOUNT_NOREORDER':
	    					if ($product->getVar('stock')=='_SHOP_MI_PRODUCTS_NOREORDER') {
	    						return array('percentage'=>$this->getVar('percentage'), 'discount_id'=>$this->getVar('discount_id'), 'shipping_id'=>$this->getVar('shipping_id'), 'amount' => ($product->getTotal($quanity) * ($this->getVar('percentage')/100)));
	    					}
	    					break;
	    				case '_SHOP_MI_DISCOUNT_QUANITY':
	    			    	if ($this->getVar('min_quanity')>=$quanity) {
	    						return array('percentage'=>$this->getVar('percentage'), 'discount_id'=>$this->getVar('discount_id'), 'shipping_id'=>$this->getVar('shipping_id'), 'amount' => ($product->getTotal($quanity) * ($this->getVar('percentage')/100)));
	    					}
	    					break;
	    				case '_SHOP_MI_DISCOUNT_OTHER':
	    					return array('percentage'=>$this->getVar('percentage'), 'discount_id'=>$this->getVar('discount_id'), 'shipping_id'=>$this->getVar('shipping_id'), 'amount' => ($product->getTotal($quanity) * ($this->getVar('percentage')/100)));
	    					break;
	    			}
		    	}
	    ...
	    return array('percentage'=>0, 'discount_id'=>0, 'shipping_id'=>0, 'amount' => 0);
    }
