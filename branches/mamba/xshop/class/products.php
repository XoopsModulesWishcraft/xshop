...
class xshopProducts extends XoopsObject 
{
    ...
    function getCatNumber() {
    	return (strlen($this->getVar('cat_prefix'))>0?$this->getVar('cat_prefix').'-':'').$this->getVar('cat_number').(strlen($this->getVar('cat_subfix'))>0?'-'.$this->getVar('cat_subfix'):'').(strlen($this->getVar('sub_model'))>0?'/'.$this->getVar('sub_model'):'');	
    }

    function getName($long=true) {
    ...
    function getDescription($long=true) {
    	$item = get_item_id($this->getVar('item_id'));
    	if (!empty($item['long_description'])&&$long==true)
    		return $item['long_description'];
    	else 
    		return $item['menu_description'];
    }
    
    function getShipping($quanity) {
    	$shipping_handler = xoops_getmodulehandler('shipping', 'xshop');
    	$shipping = $shipping_handler->get($this->getVar('shipping_id'));
    	if (is_object($shipping)) {
    		$weight = $quanity * $this->getVar('weight_per_unit');
    		switch($this->getVar('weight_measurement')) {
    			case '_SHOP_MI_WEIGHT_KILOS':
    				$cost = $weight * $shipping->getVar('price_per_kilo');
    				break;
    			case '_SHOP_MI_WEIGHT_POUNDS':
    				$cost = $weight * $shipping->getVar('price_per_pound');
    				break;
    			default:
    				$cost = $weight * $shipping->getVar('price_per_other');
    				break;
    ...
    function getTax($quanity, $ip) {
		
    	xoops_load('xoopscache');
		if (!class_exists('XoopsCache'))
			xoops_load('cache');
		
		if ($GLOBALS['xoopsModuleConfig']['autotax']) {
	    	if (!$locations=XoopsCache::read('xshop_iplocal_'.md5($ip))) {
	    		...
	    		XoopsCache::write('xshop_iplocal_'.md5($ip), $locations);
	    	}
	    	...
	    	$rate = $autotax_handler->getTaxRate(strtoupper($locations['countryCode']));
		} else {
			$rate = $GLOBALS['xoopsModuleConfig']['tax_rate'];
		}
		return ($this->getHandling($quanity)+$this->getShipping($quanity)+$this->getTotal($quanity)-$this->getDiscount($quanity)) * ($rate/100);
    }
    
	function getHandling($quanity) {
    	$shipping_handler = xoops_getmodulehandler('shipping', 'xshop');
    	$shipping = $shipping_handler->get($this->getVar('shipping_id'));
    	if (is_object($shipping)) {
    		$cost = $quanity * $shipping->getVar('handling_per_unit');
    		return $cost;
    	}
    	return 0;
    }
    
	function getTotal($quanity) {
		if ($quanity>=$this->getVar('quanity_for_wholesale')&&$this->getVar('quanity_for_wholesale')>0)
			$cost = $quanity * $this->getVar('unit_wholesale_price');
		else 
			$cost = $quanity * $this->getVar('unit_price');
    	return $cost;
    }
    
	function getDiscount($quanity) {
    	$discount_handler = xoops_getmodulehandler('discounts', 'xshop');
    	if ($quanity>=$this->getVar('quanity_for_wholesale')&&$this->getVar('quanity_for_wholesale')>0)
			$discount = $discount_handler->get($this->getVar('wholesale_discount_id'));
		else 
			$discount = $discount_handler->get($this->getVar('discount_id'));
    	return $discount->getDiscount($quanity, $this);
    }
    ...
    function toArray() {
    	...
    	$ret['cat_num'] = $this->getCatNumber();
		if ($GLOBALS['xoopsModuleConfig']['tags']) {    	
    		include_once XOOPS_ROOT_PATH."/modules/tag/include/tagbar.php";
    		$ret['tagbar'] = tagBar($this->getVar('product_id'), $catid = 0);
		}
    ...
    function getForm($querystring, $captions = true, $render = true, $index = '', $cursor = 'form', $frmobj = '', $const = '_SHOP_MI_PRODUCTS') {
    	...
    	if ($render == true||$captions==true) {
	    	...
	    	if ($GLOBALS['xoopsModuleConfig']['tags']) {
		    	$frmobj[$cursor]['tags'] = new XoopsFormTag($id.'[tags]', 35, 255, $this->getVar('product_id'));
		    	$frmobj[$cursor]['tags']->setDescription(_SHOP_FRM_PRODUCTS_TAGS_DESC);
	    	}
