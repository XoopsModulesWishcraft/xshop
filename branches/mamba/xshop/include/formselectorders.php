 ...
 * @access 		public
 */
class XShopFormSelectOrders extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XShopFormSelectOrders($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
		...
		foreach($orders_handler->getObjects($criteria, true) as $order_id => $order) {
			$this->addOption($order_id, $order->getVar('key'));
		}
    }
