...
class xshopGallery extends XoopsObject 
{
    ...
    function getForm($querystring, $captions = true, $render = true, $index = '', $cursor = 'form', $frmobj = '') {
    	...
    	if ($render = true||$captions==true) {
	    	...
	    	$frmobj[$cursor]['upload'] = new XShopFormFile(_SHOP_FRM_GALLERY_UPLOAD_FILE, $id.'[upload]', $GLOBALS['xoopsModuleConfig']['max_file_size']);
	    	...
	    	$frmobj[$cursor]['upload'] = new XShopFormFile('', $id.'[upload]', $GLOBALS['xoopsModuleConfig']['max_file_size']);
	    	if (!empty($index))	
	    		$frmobj[$cursor]['picture_id'] = new XoopsFormHidden($index.'[id]['.$this->getVar('picture_id').']', 'gallery');
	    	else 
	    		$frmobj[$cursor]['picture_id'] = new XoopsFormHidden('id['.$this->getVar('picture_id').']', 'gallery');
