...
class xshopShops extends XoopsObject 
{
    ...
    function isOrdering() {
    	if ($this->getVar('24hour_ordering'))
    		return true;
    	return $this->isOpen();
    }
    
    function isOpen() {
    	if ($this->getVar('24hour_online'))
    		return true;
...
class xshopShopsHandler extends XoopsPersistableObjectHandler
{
    ...
    function getIn($cat_id=0, $shop_id=0, $manu_id=0, $product_id=0, $is_open = true, $is_ordering = true) {
    	...
    	foreach($products_handler->getObjects($criteria, true) as $key => $object) {
    		if ($groupperm_handler->checkRight('view_shops', $object->getVar('shop_id'), $groups, $GLOBALS['xoopsModule']->getVar('mid'))) {
	    		$shop = parent::get($object->getVar('shop_id'));
	    		if (is_object($shop)) {
		    		if ($shop->isOpen()==true&&$is_open==true)
		    			$ret[$object->getVar('shop_id')] = $object->getVar('shop_id');
		    		elseif ($shop->isOrdering()==true&&$is_ordering==true)
		    			$ret[$object->getVar('shop_id')] = $object->getVar('shop_id');
		    		elseif ($is_open==false&&$is_ordering==false)
		    			$ret[$object->getVar('shop_id')] = $object->getVar('shop_id');
	    		}
    		}
    	}
