 ...
 */
class xshopField extends XoopsObject
{
    function __construct()
    {
        $this->initVar('field_id', XOBJ_DTYPE_INT, null);
        $this->initVar('manu_ids', XOBJ_DTYPE_ARRAY, array(0=>'0'), false);
        $this->initVar('shop_ids', XOBJ_DTYPE_ARRAY, array(0=>'0'), false);
        $this->initVar('cat_ids', XOBJ_DTYPE_ARRAY, array(0=>'0'), false);
    ...
    * @param xshopProfile $profile {@link xshopProfile} object to edit the value of
    *
    ...
    **/
    function getEditElement($user, $profile)
    {
        ...
        switch ($this->getVar('field_type')  ) {
            ...
            case "dhtml":
                $element = new XShopFormDhtmlTextArea($caption, $name, $value, 10, 30);
                break;
            ...
            case "select":
                $element = new XShopFormSelect($caption, $name, $value);
            ...
            case "select_multi":
                $element = new XShopFormSelect($caption, $name, $value, 5, true);
                $element->addOptionArray($options);
                break;

            case "radio":
                $element = new XShopFormRadio($caption, $name, $value);
                $element->addOptionArray($options);
                break;

            case "checkbox":
                $element = new XShopFormCheckBox($caption, $name, $value);
                $element->addOptionArray($options);
                break;
            ...
            case "group":
                $element = new XShopFormSelectGroup($caption, $name, true, $value);
                break;

            case "group_multi":
                $element = new XShopFormSelectGroup($caption, $name, true, $value, 5, true);
                break;

            case "language":
                $element = new XShopFormSelectLang($caption, $name, $value);
                break;
            ...
            case "datetime":
                $element = new XShopFormDatetime($caption, $name, 15, $value);
                break;

            case "list":
                $element = new XShopFormSelectList($caption, $name, $value, 1, $options[0]);
                break;

            case "timezone":
                $element = new XShopFormSelectTimezone($caption, $name, $value);
    ...
    * @param xshopProfile $profile object to get the value of
    *
    * @return mixed
    **/
    function getOutputValue($user, $profile)
    {
        if (file_exists($file = $GLOBALS['xoops']->path('modules/xshop/language/' . $GLOBALS['xoopsConfig']['language'] . '/modinfo.php'))) {
            include_once $file;
        } else {
            include_once $GLOBALS['xoops']->path('modules/xshop/language/english/modinfo.php');
        }
    ...
    */
    ...
    {
        $xshop_handler = xoops_getmodulehandler('extras', 'xshop');
        return $xshop_handler->getPostVars();
    }
}
 ...
 * @copyright copyright &copy; 2000 XOOPS.org
 */
class xshopFieldHandler extends XoopsPersistableObjectHandler
{
    function __construct($db)
    {
        parent::__construct($db, 'xf_field', "xshopField", "field_id", 'field_title');
    }
    ...
    */
    ...
    {
        $xshop_handler =& xoops_getmodulehandler('extras', 'xforum');
        ...
        if (!in_array($obj->getVar('field_name'), $this->getPostVars())) {
            ...
            $sql = "ALTER TABLE `" . $xshop_handler->table . "` " .
    ...
    **/
    function delete($obj, $force = false)
    {
        ...
        $sql = "ALTER TABLE " . $xshop_handler->table . " DROP `" . $obj->getVar('field_name', 'n') . "`";
        if ($this->db->query($sql)) {
            ...
            if ($obj->getVar('field_show') || $obj->getVar('field_edit')) {
                ...
                $xshop_module = $module_handler->getByDirname('xshop');
                if (is_object($xshop_module)) {
                    // Remove group permissions
                    $groupperm_handler =& xoops_gethandler('groupperm');
                    $criteria = new CriteriaCompo(new Criteria('gperm_modid', $xshop_module->getVar('mid')));
