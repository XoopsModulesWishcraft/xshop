 ...
 * @access 		public
 */
class XoopsFormSelectDiscount extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectDiscount($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
		...
		foreach($discounts_handler->getObjects($criteria, true) as $discounts_id => $item) {
			$digest = $item_digest_handler->getItem($item->getVar('items_id'));
			$this->addOption($discounts_id, $digest->getVar('menu_title'));
		}
    }
