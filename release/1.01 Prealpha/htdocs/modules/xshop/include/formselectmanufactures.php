 ...
 * @access 		public
 */
class XoopsFormSelectManufactures extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectManufactures($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
		...
		foreach($manufactures_handler->getObjects($criteria, true) as $manufactures_id => $item) {
			$digest = $item_digest_handler->getItem($item->getVar('items_id'));
			$this->addOption($manufactures_id, $digest->getVar('menu_title'));
		}
    }
