 ...
 * @access 		public
 */
class XoopsFormSelectCurrency extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectCurrency($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
        ...
        $criteria->setSort('`iso_code`');
        $item_digest_handler = xoops_getmodulehandler('item_digest', 'xshop');
		foreach($currency_handler->getObjects($criteria, true) as $currency_id => $item) {
			$digest = $item_digest_handler->getItem($item->getVar('items_id'));
			$this->addOption($currency_id, $item->getVar('iso_code') . ' ' .$digest->getVar('menu_title'));
		}
    }
