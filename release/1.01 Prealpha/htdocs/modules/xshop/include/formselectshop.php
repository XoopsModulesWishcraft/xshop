 ...
 * @access 		public
 */
class XoopsFormSelectShops extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectShops($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
		...
		foreach($shops_handler->getObjects($criteria, true) as $shops_id => $item) {
			$digest = $item_digest_handler->getItem($item->getVar('items_id'));
			$this->addOption($shops_id, $digest->getVar('menu_title'));
		}
    }
