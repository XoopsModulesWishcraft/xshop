...
class xshopProducts extends XoopsObject 
{
    ...
    function getForm($querystring, $captions = true, $render = true, $index = '', $cursor = 'form', $frmobj = '', $const = '_SHOP_MI_PRODUCTS') {
    	...
    	if ($render == true||$captions==true) {
	    	...
	    	$frmobj[$cursor]['sub_model'] = new XoopsFormText('', 'sub_model', 12, 35, $this->getVar('sub_model'));
	    	$frmobj[$cursor]['unit_price'] = new XoopsFormText('', 'unit_price', 12, 35, $this->getVar('unit_price'));
	    	$frmobj[$cursor]['unit_wholesale_price'] = new XoopsFormText('', 'unit_wholesale_price', 12, 35, $this->getVar('unit_wholesale_price'));
	    	$frmobj[$cursor]['weight_per_unit'] = new XoopsFormText('', 'weight_per_unit', 12, 35, $this->getVar('weight_per_unit'));
	    	$frmobj[$cursor]['weight_measurement'] = new XShopFormSelectMeasurement('', 'weight_measurement', $this->getVar('weight_measurement'));
	    	$frmobj[$cursor]['quanity_in_unit'] = new XoopsFormText('', 'quanity_in_unit', 12, 35, $this->getVar('quanity_in_unit'));
	    	$frmobj[$cursor]['quanity_for_wholesale'] = new XoopsFormText('', 'quanity_for_wholesale', 12, 35, $this->getVar('quanity_for_wholesale'));
	    	$frmobj[$cursor]['quanity_in_warehouse'] = new XoopsFormText('', 'quanity_in_warehouse', 12, 35, $this->getVar('quanity_in_warehouse'));
	    	$frmobj[$cursor]['quanity_to_order'] = new XoopsFormText('', 'quanity_to_order', 12, 35, $this->getVar('quanity_to_order'));
